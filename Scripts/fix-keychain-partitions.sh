#!/bin/bash
# Add PenguinNotch's Team ID to the partition list of every Claude Code keychain
# item.
#
...
#
# PenguinNotch used to be ad-hoc signed, so its identity was a cdhash that changed
...
TEAM_ID="$(codesign -dv /Applications/PenguinNotch.app 2>&1 | sed -nE 's/^TeamIdentifier=([A-Z0-9]{10})$/\1/p')"
if [ -z "$TEAM_ID" ] || [ "$TEAM_ID" = "not" ]; then
  echo "PenguinNotch is not signed with a Team ID — rebuild with 'make install' first." >&2
  exit 1
fi
...
# apple-tool: would lock *them* out to let PenguinNotch in.
...
if [ "$failed" -eq 0 ]; then
  ...
  echo "PenguinNotch should stop asking for the password."
else
