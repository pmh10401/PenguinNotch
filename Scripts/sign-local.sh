...
#
# Usage:
#   Scripts/sign-local.sh [path/to/PenguinNotch.app]
# Default target is the app in /Applications.

set -euo pipefail

APP="${1:-/Applications/PenguinNotch.app}"
CN="PenguinNotch Local Signing"
...
EOF
  ...
  openssl pkcs12 -export -legacy -inkey "$DIR/key.pem" -in "$DIR/cert.pem" \
    -out "$DIR/id.p12" -passout pass:penguinnotch -name "$CN" 2>/dev/null || \
  openssl pkcs12 -export -inkey "$DIR/key.pem" -in "$DIR/cert.pem" \
    -out "$DIR/id.p12" -passout pass:penguinnotch -name "$CN"
  ...
  security import "$DIR/id.p12" -k "$HOME/Library/Keychains/login.keychain-db" \
    -P penguinnotch -T /usr/bin/codesign -A
fi

# Quit a running instance so the bundle can be replaced in place.
osascript -e 'quit app "PenguinNotch"' 2>/dev/null || true
sleep 1
...
echo
echo "Signed with '$CN'. Relaunch PenguinNotch and grant the keychain prompt one"
