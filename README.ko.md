...
**Stocks**는 Settings → Stocks에 추가한 종목의 최근 체결가를 보여줍니다. 한국 종목 코드(`005930`)와 미국 티커(`AAPL`)를 섞어 최대 30개를 등록할 수 있습니다. 토스증권을 선택한 경우 현재가는 `GET /api/v1/prices`에서 읽고 이후 체결은 `wss://openapi-ws.tossinvest.com/ws/v1`에서 받습니다. `Client ID`와 `Client secret`은 토스증권 WTS의 **설정 → Open API**에서 발급받고 macOS 키체인에 저장합니다. 같은 화면의 허용 IP에 이 Mac의 공인 IP를 등록해야 합니다. 종목마다 노치 항목이 하나씩 생깁니다. **Settings → Appearance → Meter style**에서 AI 계정·시스템 항목·주식을 원이나 가로 막대로 표시할 수 있습니다. 주식 변화량은 전일 종가 대비이며 30% 변화가 원이나 막대를 채웁니다. 상승은 초록, 하락은 빨강으로 표시합니다. 노치에는 등락률과 현재가가 기본 3초마다 교대로 나오며 **Settings → Stocks → 노치 시세 표시**에서 1~10초로 조절할 수 있습니다. 전일 종가를 조회하지 못하면 등락률은 `—`로 표시하고 호버 카드에는 현재가를 유지합니다. 조용한 시장에서는 마지막 가격을 유지합니다. 비밀키는 일반 설정에 기록하지 않습니다.

**주식 예측과 성과 기록(macOS, 토스)**은 **설정 → 주식 → 내 토스 보유 종목**에서 사용합니다. 실제 보유 종목 추정을 켠 뒤 **예측 자동 기록 및 채점**을 켜거나 **지금 예측 저장**을 누르세요. GBM 모형의 80% 예상 가격 범위도 표시합니다. 확률과 범위는 검증·보정되지 않았으며, 대화에서 받은 GPT/Grok 응답을 자동으로 가져오거나 생성하는 기능은 아닙니다.

- 자동 기록은 정규장 마감 55~60분 전의 첫 예측을 종목·거래일·모형 버전별로 한 번 저장합니다. 앱이 실행 중이고 토스 보유 종목 추정과 자동 기록이 켜져 있어야 합니다. 놓친 예측을 사후에 만들지 않으며 입력 체결가는 2분 이내여야 합니다.
- 수동 기록도 종목·거래일·모형별 하루 한 번 저장하고 자동 기록과 따로 집계합니다. 저장한 예측값은 바꾸지 않습니다. 시장 현지 날짜가 다음 날이 되면 정확히 해당 거래일의 수정주가 미적용 토스 일봉 종가와 비교합니다. 데이터가 없으면 대기 상태를 유지하며 다른 날짜의 종가를 대신 쓰지 않습니다. 앱을 다시 열면 대조를 재개하고, 자동 기록을 껐다면 주식 설정을 열어 대조합니다.
- **예측 기록과 성과 보기**에서 종목·모형·기간·기록 시점별로 걸러 보고 해당 목록을 CSV로 내보냅니다. 표본 수와 함께 방향 적중률, 평균 가격 오차(MAPE), 현재가 유지 기준선 오차, 80% 예상 범위 포함률, Brier 점수를 표시합니다. 방향은 전일 종가보다 높은지 낮은지이며 보합과 상승 확률 50%인 예측은 방향 채점에서 제외합니다. Brier는 ‘전일 종가보다 상승했는가’를 평가하므로 보합은 상승하지 않은 결과입니다. GBM 예상 종가는 현재가와 같아 가격 오차도 기준선과 같습니다. 이 지표는 투자 수익률이 아닙니다.

기록은 `~/Library/Application Support/PenguinNotch/Forecasts/history.json`에 저장합니다. 종목·이름·가격·시각·모형 결과를 보관하고 계좌번호·보유 수량·잔고·API 비밀키는 넣지 않습니다. 자동 기록은 기본적으로 꺼져 있으며 꺼도 기존 기록은 남습니다. 토스 외의 제공처로 바꾸거나 보유 종목 추정을 끄면 관련 요청을 멈춥니다. 읽지 못한 기록 파일은 덮어쓰지 않고 보존합니다.

**Settings → Stocks → Quote provider**에서 **제공처 하나만 선택**합니다. 선택한 업체의 설정과 API만 사용하며, 업체를 바꿔도 관심 목록과 저장된 키는 유지합니다. **토스증권**은 한국·미국 시세와 호버 봉 차트를 지원합니다. 무료 미국 시세를 보려면 **Finnhub**를 고르고 [개인 API 키를 발급](https://finnhub.io/register)받아 macOS 키체인에 저장하세요. 미국 종목마다 `GET /api/v1/quote`로 현재가와 전일 종가를 읽고 약 1분마다 갱신하며, 키는 요청 헤더로만 전송합니다. Finnhub 모드에서는 토스 API를 호출하지 않습니다. 한국 종목은 목록에 유지하되 미지원 안내를 표시하고 봉 차트도 제공하지 않습니다. Finnhub 호버 카드에는 시세 정보만 표시하며 빈 차트 공간을 남기지 않습니다. 차트는 토스를 다시 선택하면 이용할 수 있습니다. 기존 제공처 선택을 유지하며 기본값은 토스입니다. Finnhub 요금제 호출 한도와 [개인 사용 약관](https://finnhub.io/terms-of-service)이 적용되며, 조회 실패를 0원 시세로 표시하지 않습니다.
