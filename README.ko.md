...
- 현재 보유 종목과 저장 기록의 **예측 근거**에서 입력 체결가, 시장 현지 시각, 목표 종가 시각, 당시의 수정주가가 적용된 완료 일봉, 일간 로그수익률 변동성, 과거 5·20거래일 수익률을 확인합니다. 과거 수익률은 참고 정보이며 GBM은 계속 기대수익률을 0으로 가정합니다. 새 기록에는 일봉도 저장하고, 이전 기록은 근거 일봉을 저장하지 않았다고 표시합니다. CSV에도 저장한 입력을 포함합니다.
- **모형 비교**는 나열된 모든 모형에서 공통으로 채점된 기록만 사용합니다. 종목, 체결 시각, 가격, 정규장 시간, 기록 방식, 근거 일봉이 같아야 하며 대기·불일치 기록은 비교 MAPE/Brier에서 제외합니다. 이전 기록도 해당 기록의 현재가 유지 기준선과는 비교할 수 있습니다. 비교 탭에서 CSV를 내보내면 선택한 필터의 모든 모형과 대기·불일치 기록이 포함됩니다.
- **확률 검증**은 선택한 모형의 채점된 예측을 10%p 구간으로 묶어 평균 예측 확률, 실제 상승 비율, 표본 수, 참고용 95% Wilson 구간을 표시합니다. 보합은 상승하지 않은 결과로, 50% 예측도 표본으로 포함하며 빈 구간은 생략합니다. 종목·날짜 사이의 상관관계에 따라 실제 불확실성은 더 클 수 있습니다. 예측의 신뢰도를 확인하는 기능이며 모형을 자동 학습시키지는 않습니다.

현재 버전은 **GBM을 로컬에서 실행**합니다. **Laya와 Jev는 아직 연결되지 않았으며**, 유료 AI 호출이나 뉴스 수집을 추가하지 않습니다. 근거 표시와 검증은 기존 토스 데이터를 재사용하고 완료 일봉은 해당 정규장 세션 동안 캐시합니다.
