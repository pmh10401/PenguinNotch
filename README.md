...
**Stock predictions and evaluation (macOS, Toss)** are under **Settings → Stocks →
My Toss holdings**. Enable actual-holdings estimates, then opt in to **Automatically
record and evaluate forecasts**, or use **Save current predictions**. The local GBM
...
- Automatic recording saves the first prediction in the 55–60 minutes before the
  ...
  running with Toss holdings estimates and recording enabled. Missed predictions
...
- **Prediction evidence** in current holdings and saved records shows the exact input
...
This version runs **GBM locally**. **Laya and Jev are not connected**, and no paid AI
calls or news ingestion are introduced. Evidence and evaluation reuse the existing
Toss data; completed daily candles remain cached for the regular session.
...
holdings estimates stops its requests. An unreadable archive is preserved, not replaced.
