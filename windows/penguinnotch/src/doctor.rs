//! `penguinnotch.exe doctor` — self-diagnosis: look instead of guessing.
//! Checks the config, port occupancy, watch roots, the newest session file and how its tail parses,
//! and writes to stdout plus %APPDATA%\penguinnotch\doctor.log.
...
pub fn run() -> String {
    ...
    o += &format!("== PenguinNotch doctor v{} ==\n", env!("CARGO_PKG_VERSION"));
    ...
    match std::net::TcpListener::bind(("127.0.0.1", cfg.port)) {
        Ok(_) => o += "port: free — no PenguinNotch instance is running\n",
    ...
    if let Some(dir) = dirs::config_dir() {
        let p = dir.join("penguinnotch").join("watch.log");
