//! Merges penguinnotch-hook.exe into ~/.claude/settings.json without overwriting the user's own hooks.
//! Identification: the command contains "penguinnotch-hook". A backup is written first.

use serde_json::{json, Value};
use std::path::PathBuf;

/// (Claude Code event name, whether it needs a matcher, the internal event reported to PenguinNotch)
        ...
        .map(|hs| {
            hs.iter().any(|h| {
                h["command"]
                    .as_str()
                    .map(|c| {
                        ["penguinnotch-hook", "codenotch-hook", "eatbean-hook", "pacman-hook"]
                            .iter()
...
fn backup_and_write(path: &PathBuf, root: &Value) -> Result<(), String> {
    ...
    if path.exists() {
        let ts = std::time::SystemTime::now()
            ...
            .map(|d| d.as_secs())
            .unwrap_or(0);
        let _ = std::fs::copy(path, path.with_extension(format!("json.penguinnotch-bak-{ts}")));
    }
...
pub fn is_installed() -> bool {
    settings_path()
        ...
        .map(|t| t.contains("penguinnotch-hook") || t.contains("codenotch-hook"))
        .unwrap_or(false)
}
...
pub fn uninstall() -> Result<String, String> {
    ...
    backup_and_write(&path, &root)?;
    Ok(format!("removed {removed} PenguinNotch hook(s)"))
}
