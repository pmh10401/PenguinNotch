...
fn ack_scan(app: &AppHandle) -> bool {
    ...
    let fg_is_claude_desktop = fg_name.contains("claude") && !fg_name.contains("penguinnotch");
    let st = app.state::<AppState>();
    let mut store = st.store.lock().unwrap();
    store.ack_done(|s| {
        if s.ppid == 0 {
...
fn main() {
        ...
        match cmd.as_str() {
            ...
            "autostart" => {
                let r = match args.get(2).map(|s| s.as_str()) {
                    ...
                    _ => Err("usage: penguinnotch.exe autostart on|off".into()),
                };
                report(r);
                return;
            }
    ...
    tauri::Builder::default()
        ...
        .plugin(tauri_plugin_single_instance::init(|app, _args, _cwd| {
            // Opening PenguinNotch again while it runs brings Settings forward, as on the Mac: with the
        ...
        .setup(move |app| {
            ...
            // Persist the config (penguinnotch-hook reads the port from it)
            {
        ...
        .expect("PenguinNotch failed to start");
}
