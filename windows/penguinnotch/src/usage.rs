//! Claude usage adapter (official), implemented from the upstream PenguinNotch's documented behaviour.
...
fn run_renewal(cli: &std::path::Path, dir: &Path) -> std::io::Result<()> {
    ...
    // survives the loop above, and a PenguinNotch started from a shell pointed at another account used to renew
