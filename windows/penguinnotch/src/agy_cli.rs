...
mod tests {
    ...
    #[test]
    fn test_missing_cli() {
        let temp = std::env::temp_dir().join(format!("penguinnotch-empty-test-{}", std::process::id()));
    ...
    fn test_atomic_write_preserves_old_on_failure() {
        let dir = std::env::temp_dir().join(format!("penguinnotch-atomic-test-{}", std::process::id()));
