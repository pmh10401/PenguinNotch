...
const LOGIN_SCRIPT: &str = "$Host.UI.RawUI.WindowTitle = 'PenguinNotch - Claude sign-in'; Write-Host 'Complete sign-in in your browser. Paste any code in this window.'; & $env:PENGUINNOTCH_CLAUDE_CLI auth login --claudeai; $loginResult = $LASTEXITCODE; if ($loginResult -eq 0) { Write-Host 'Sign-in complete. PenguinNotch will refresh automatically.'; Start-Sleep -Seconds 2 } else { Write-Host 'Sign-in failed or cancelled. Retry from PenguinNotch.'; Start-Sleep -Seconds 8 }; exit $loginResult";
...
fn login_command(cli: &std::path::Path) -> Result<Command, String> {
    ...
    cmd.args(["-NoLogo", "-NoProfile", "-Command", LOGIN_SCRIPT]).env("PENGUINNOTCH_CLAUDE_CLI", cli);
...
mod tests {
    ...
    fn paths_are_data_not_shell_source() {
        ...
        assert!(cmd.get_envs().any(|(k,v)| k == "PENGUINNOTCH_CLAUDE_CLI" && v == Some(path.as_os_str())));
    }
    #[test]
    #[cfg(windows)]
