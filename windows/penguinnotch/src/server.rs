//! Local event server: receives penguinnotch-hook's POST /event?e=<event>&ppid=<pid>
...
pub fn start(app: AppHandle, port: u16) {
    std::thread::spawn(move || {
        let server = match tiny_http::Server::http(("127.0.0.1", port)) {
            Ok(s) => s,
            Err(e) => {
                eprintln!("[penguinnotch] failed to bind port {port}: {e} (is another instance running?)");
                return;
            }
        };
        for mut req in server.incoming_requests() {
            ...
            if url.starts_with("/event") {
                // penguinnotch-hook only ever POSTs. A GET is also what a web page can send with no
...
///
/// Native callers like `penguinnotch-hook` communicate directly over loopback TCP without
