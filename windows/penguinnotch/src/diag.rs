//! `penguinnotch.exe doctor deep`: deep diagnostics for finding "is it working?" signals.
...
mod tests {
    ...
    fn dump_sqlite_keeps_columns_but_not_values() {
        let path = std::env::temp_dir().join(format!("penguinnotch-diag-{}-sqlite.db", std::process::id()));
    ...
    fn json_scalars_render_as_type_and_length_only() {
        let path = std::env::temp_dir().join(format!("penguinnotch-diag-{}-state.json", std::process::id()));
        std::fs::write(
            &path,
