//! Codex usage adapter, implemented from the upstream PenguinNotch's documented behaviour.
//!
...
//!      Codex owns any managed OAuth refresh; PenguinNotch sends no login/refresh request itself.
...
fn fetch_usage(cred: &Credential) -> Result<serde_json::Value, LiveErr> {
    let resp = ureq::get(ENDPOINT)
        ...
        .set("User-Agent", concat!("penguinnotch/", env!("CARGO_PKG_VERSION"), " (Windows)"))
...
fn read_app_server() -> Option<UsageSnapshot> {
    ...
    let result = (|| {
        ...
        writeln!(input, "{}", serde_json::json!({"id":1,"method":"initialize","params":{"clientInfo":{"name":"penguinnotch","version":env!("CARGO_PKG_VERSION")}}})).ok()?;
        input.flush().ok()?;
...
mod tests {
    ...
    fn resumed_thread_in_old_date_directory_is_found() {
        let root = std::env::temp_dir().join(format!("penguinnotch-rollout-test-{}-{}", std::process::id(), now_ms()));
    ...
    fn rollout_index_skips_missing_paths_and_accepts_legacy_timestamp_column() {
        let root = std::env::temp_dir().join(format!("penguinnotch-index-test-{}-{}", std::process::id(), now_ms()));
        std::fs::create_dir(&root).unwrap();
    ...
    fn corrupt_rollout_index_uses_bounded_directory_fallback() {
        let root = std::env::temp_dir().join(format!("penguinnotch-index-fallback-{}-{}", std::process::id(), now_ms()));
