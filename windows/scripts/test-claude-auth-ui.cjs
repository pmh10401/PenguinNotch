...
(async()=>{
  ...
  assert.equal((ctx.claudeAuthHtml({busy:true,message:''}).match(/disabled/g)||[]).length,2);
  ...
  await ctx.runClaudeAction('refresh');
  assert.deepEqual(calls,['refresh_claude_usage']);
  await ctx.runClaudeAction('sign-in');
  await ctx.runClaudeAction('sign-in');
  assert.deepEqual(calls,['refresh_claude_usage','claude_sign_in']);
  ctx.claudeAuth.busy=false;
  ctx.invoke=async()=>false;
  await ctx.runClaudeAction('refresh');
  assert.match(ctx.claudeActionMessage,/Rate limited/);
  ...
  console.log('PASS: auth labels/localization, busy guard, escaping, provider refresh/backoff and launch errors');
