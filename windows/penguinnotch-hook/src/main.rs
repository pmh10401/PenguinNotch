//! penguinnotch-hook: the minimal client Claude Code's hooks call.
...
/// Pulls "port": N out of %APPDATA%\penguinnotch\config.json (hand-rolled scan, no dependency)
fn read_port() -> u16 {
    let path = match std::env::var("APPDATA") {
        Ok(a) => format!("{a}\\penguinnotch\\config.json"),
...
fn spawn_main() {
    ...
    let exe = dir.join("penguinnotch.exe");
    if !exe.exists() {
        return;
    }
