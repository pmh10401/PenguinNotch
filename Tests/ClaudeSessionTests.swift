...
/// The session PenguinNotch starts itself must never reach the notch.
///
...
/// carries the pid of the process PenguinNotch spawned. Left alone it draws a row
...
final class ClaudeOwnSessionFilterTests: XCTestCase {
    ...
    func testAnIgnoredPidIsLeftOut() throws {
        ...
        XCTAssertTrue(found.isEmpty, "the session PenguinNotch started is not the user's")
    }
