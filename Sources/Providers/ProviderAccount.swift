...
///
/// Worth showing plainly, because PenguinNotch never signs in — it borrows a
...
///
/// PenguinNotch cannot sign anyone in — it reads a credential the owning tool
...
enum SignInRoute: Equatable {
    ...
    /// The only case where PenguinNotch genuinely signs anyone in or out.
    ...
    var signOutCaveat: String {
        switch self {
        case .modal(let name):
            return L10n.t("Signs out of \(name) — the session belongs to PenguinNotch.")
        case .openApp(_, let name):
...
struct ProviderSummary: Identifiable, Equatable {
    ...
    /// Whether this provider's saved login has aged out and PenguinNotch could not
