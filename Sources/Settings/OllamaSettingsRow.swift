    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Toggle(L10n.t("Measure speed and thinking"), isOn: $preferences.ollamaMetricsEnabled)
                .disabled(!enabled)
            Text(L10n.t("To measure responses, point your chat client to http://127.0.0.1:11435 and keep PenguinNotch open."))
                .foregroundStyle(.secondary)
        ...
        VStack(alignment: .leading, spacing: 6) {
            ...
            if relay.ready {
                ...
                Text("OLLAMA_HOST=\(OllamaActivityRelay.address) ollama")
                    ...
                    .textSelection(.enabled)
                Text("Keep PenguinNotch open. Speed appears after each completed native Ollama response. Requests sent directly to the server address only provide model detection here.")
                    .foregroundStyle(.secondary)
            }
        }
    }
