# PenguinNotch — Implementation Plan
...
- [ ] `project.yml` for XcodeGen: app target `PenguinNotch` + unit test target
- [ ] `Makefile` with `gen` / `build` / `test` / `run` / `clean`
- [ ] `Sources/Info.plist` with `LSUIElement: true`, min system 26.0
- [ ] `Sources/App/PenguinNotchMain.swift` + `AppDelegate` that launches with no window
...
1. **Name.** `PenguinNotch` is a placeholder.
