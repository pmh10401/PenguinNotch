...
PenguinNotch for macOS reads the Amp CLI login from
`~/.local/share/amp/secrets.json`. Run `amp login`, then refresh PenguinNotch.
Enable or disable Amp in **Settings → Accounts** like any other provider.
Disabling it stops polling and forgets PenguinNotch's readings; it leaves the
Amp login file untouched.

## Readings

- **Subscription:** Agent usage is the headline ring. Orb usage is a separate
  tooltip row. Amp reports percentages remaining; PenguinNotch shows the
...
restart. No credential is copied, refreshed, logged, or written by PenguinNotch.
...
`amp usage` in Terminal to let the CLI renew its token, then refresh PenguinNotch.
...
use PenguinNotch's normal last-good-reading/stale behavior.
